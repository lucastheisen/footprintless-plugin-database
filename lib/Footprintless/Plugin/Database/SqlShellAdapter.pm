...
sub sql_shell {
    ...
    if (-t STDIN) { ## no critic
        ...
        ($ENV{EDITOR}) = $ENV{PERL_READLINE_MODE} 
            ? $ENV{PERL_READLINE_MODE}
            : $ENV{SHELLOPTS} 
                ? $ENV{SHELLOPTS} =~ /\b(emacs|vi)\b/
                : $ENV{EDITOR};
        ...
        $sqlsh->set('GetHistory',  
            sub {
                return [$term->GetHistory()];
            });
        $sqlsh->set('SetHistory',  
            sub {
                my $history = shift; 
                $term->SetHistory(@$history);
            });
        $sqlsh->set('AddHistory',  
            sub {
                my $cmd = shift; 
...
=func sql_shell($connection_string, $username, $password, %options)
