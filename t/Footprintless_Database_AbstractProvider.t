...

{
    ...
    eval {
        $db->connect();
        $db->query("select char_col, number_col, string_col from test",
            sub {
                push(@got, \@_);
            });
        $db->query(
...

{
    ...
    is_deeply($got_default, scalar(@{$spec->{test}{values}}), 
...

{
    ...
    is_deeply($got_arrayref, $spec->{test}{values}, 
        'query_for_list result arrayref');
}

{
    ...
    eval {
        $db->connect();
        %got_default = $db->query_for_map(
            {
                sql => "select char_col, number_col, string_col from test"
            });
        $got_hashref_default = $db->query_for_map(
            {
                sql => "select char_col, number_col, string_col from test"
            });
